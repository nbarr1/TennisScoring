...
- **Team standings** — `divisions/{divisionId}/doublesRankings/{rankingId}` holds one row per fixed partnership, season, and division level, written by `recalculateRankings()` via `computeDoublesRankings()`. `rankingId` and `teamId` are opaque, collision-safe encodings; use the stored `seasonId`, `divisionLevelId`, and `playerIds` fields rather than parsing document IDs. Ranks and head-to-head tiebreaks are computed independently within each season/level bucket. The collection is a sibling of `rankings` so the two prune independently.
