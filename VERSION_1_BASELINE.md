...
- **Team standings** — `divisions/{divisionId}/doublesRankings/{seasonId}__{teamId}` holds one row per fixed partnership per season, written by `recalculateRankings()` via the new `computeDoublesRankings()`. A team is identified by the sorted pair of its members' user ids (`doublesTeamId`), so a partnership accumulates one row per season with no registration step, and ranks are computed within each season. The collection is a sibling of `rankings` so the two prune independently.
