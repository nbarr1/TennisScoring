...
export default function DivisionOnboardingScreen() {
  ...
  async function handleJoin() {
    ...
    setLoading(true);
    try {
      const { divisionId, divisionName: name } = await joinDivisionByCode(inviteCode.trim(), firebaseUser.uid);
      ...
      const msg = err instanceof Error ? err.message : 'Could not join division.';
        ...
        {mode === 'choose' && (
          <View style={styles.choices}>
            <TouchableOpacity style={styles.choiceCard} onPress={() => setMode('create')}>
              ...
              <Text style={styles.choiceBody}>Start a new league division and invite players.</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.choiceCard} onPress={() => setMode('join')}>
              <Text style={styles.choiceIcon}>🔗</Text>
              <Text style={styles.choiceTitle}>Join Division</Text>
              <Text style={styles.choiceBody}>Enter an invite code from your division leader.</Text>
            </TouchableOpacity>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  choiceTitle: { fontSize: 18, fontWeight: '700', color: '#1a472a', marginBottom: 6 },
