...
async function createAndStoreReport(matchId: string, match: Match) {
  ...
  // Footer
  y = 40;
  page.drawText('Generated by Tennis League App', { x: 60, y, size: 9, font, color: rgb(0.5, 0.5, 0.5) });
  ...
  await getFirestore().collection('matches').doc(matchId).update({ reportUrl: signedUrl });
}
