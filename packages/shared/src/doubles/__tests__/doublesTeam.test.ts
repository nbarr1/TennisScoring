...
describe("doublesTeamId", () => {
  ...
  it("sorts member ids", () => {
    expect(doublesTeamPlayerIds(doublesTeamId(["bob", "ann"]))).toEqual([
      "ann",
      "bob",
    ]);
  });
  ...
  it("distinguishes different partnerships that share a player", () => {
    expect(doublesTeamId(["ann", "bob"])).not.toBe(
      doublesTeamId(["ann", "cara"]),
    );
  });

  it("dedupes and drops blank ids", () => {
    expect(
      doublesTeamPlayerIds(doublesTeamId(["ann", "", "  ", "ann"])),
    ).toEqual(["ann"]);
    expect(doublesTeamId([])).toBe("");
  });

  it("trims surrounding whitespace", () => {
    expect(doublesTeamId([" ann ", "bob"])).toBe(doublesTeamId(["ann", "bob"]));
  });

  it("does not collide when user ids contain separators", () => {
    expect(doublesTeamId(["a", "b_c"])).not.toBe(doublesTeamId(["a_b", "c"]));
    expect(doublesTeamPlayerIds(doublesTeamId(["a", "b_c"]))).toEqual([
      "a",
      "b_c",
    ]);
  });
...
describe("formatDoublesTeamName", () => {
  it("joins names in the order given", () => {
    expect(formatDoublesTeamName(["Ann Smith", "Bob Jones"])).toBe(
      "Ann Smith / Bob Jones",
    );
  });
...
describe("doublesHeadToHeadId", () => {
  ...
  it("scopes ids to a division level", () => {
    expect(
      doublesHeadToHeadId("team-a", "team-b", "spring-2026", "level-a"),
    ).not.toBe(
      doublesHeadToHeadId("team-a", "team-b", "spring-2026", "level-b"),
    );
  });
...
describe("matchSides accessors", () => {
  ...
  it("does not treat a two-player match tagged matchType doubles as doubles", () => {
    ...
    const mislabelled: MatchSidesLike = {
      ...singlesMatch,
      matchType: "doubles",
    };
  ...
  it("treats a four-player match with no matchType label as doubles", () => {
    const unlabelled: MatchSidesLike = {
      ...doublesMatch,
      matchType: undefined,
    };
  ...
  it("reads both partners off a doubles side", () => {
    ...
    expect(sideDisplayName(doublesMatch, "player2")).toBe(
      "Cara Lee / Dan Ruiz",
    );
