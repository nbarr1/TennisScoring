...
- **The watch is remotely launchable, and that is a two-artifact contract.** `:wear`'s `MainActivity` carries a second intent filter (`VIEW` + `DEFAULT` + `BROWSABLE`, scheme `tennisleague`) and `launchMode="singleTask"`, and `wear/src/main/res/values/wear.xml` advertises the capability `tennis_league_wear_app`. Both exist for a phone-side "open on watch" control: `RemoteActivityHelper` can only deliver an `ACTION_VIEW` intent carrying a data URI and `CATEGORY_BROWSABLE`, so an intent filter that does not match that exact shape resolves to nothing on the watch and the launch silently no-ops, and `NodeClient` reports only that a watch is connected, never that this app is installed on it — the capability is what tells the two apart. Nothing compiles or verifies either string across the module boundary, so changing the scheme or the capability name breaks the feature silently.
